(function() {
  var app = {
    routes: {
      button2: {
        rendered: function() {
          console.log('view currently showing is "HTML5"');
          app.preventScroll();
        }
      },
      button3: {
        rendered: function() {
          console.log('view currently showing is "CSS"');
          app.preventScroll();
        }
      },
      button4: {
        rendered: function() {
          console.log('view currently showing is "JavaScript');
          app.preventScroll();
        }
      },
      button5: {
        rendered: function() {
          console.log('view currently showing is "C++"');
          app.preventScroll();
        }
      },
      "the-default-view": {
        rendered: function() {
          console.log('view currently showing is "the-default-view"');
          app.preventScroll();
        }
      }
    },
    default: "the-default-view",
    preventScroll: function() {
      document.querySelector("html").scrollTop = 0;
      document.querySelector("body").scrollTop = 0;
    },
    routeChange: function() {
      app.routeID = location.hash.slice(1);
      app.route = app.routes[app.routeID];
      app.routeElem = document.getElementById(app.routeID);
      app.route.rendered();
    },
    init: function() {
      window.addEventListener("hashchange", function() {
        app.routeChange();
      });
      if (!window.location.hash) {
        window.location.hash = app.default;
      } else {
        app.routeChange();
      }
    }
  };
  window.app = app;
})();

app.init();
